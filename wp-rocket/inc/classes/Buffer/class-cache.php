...
class Cache extends Abstract_Buffer {
	...
	public function maybe_process_buffer( $buffer ) {
		 ...
		 * Save Cache into DB.
		 *
		 * @param bool $cache_dir_path Cache dir path.
		 * @param bool $request_uri    Request URI for this cache.
		 */
		$vars = [
			'url'     => $this->tests->get_clean_request_uri(),
			'path'    => $cache_dir_path,
			'user_id' => get_current_user_id(),
		];
		apply_filters( 'rocket_cache_db', $vars );
